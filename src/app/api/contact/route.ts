...
export const POST = async (req: NextRequest) => {
  ...
  //   Checking if client sent tokens or not
  if (!headers.get("x-auth-token") || !headers.get("x-refresh-token")) {
    return res.json({ message: "شما اجازه دسترسی ندارید!" }, { status: 401 });
  }
  ...
  //   Verifying tokens
  verifyTokens(headers)
    .then(async ({ email, newToken }) => {
      try {
        await dbConnect();
      } catch (err) {
        return res.json({
      ...
      return res.json(
        ...
        { status: 201, headers: { "x-auth-token": newToken && newToken } }
      );
    })
    .catch((err) => {

...
const layout = ({ children }: { children: React.ReactNode }) => {
  return (
    <main className="w-full min-h-[600px] h-screen bg-zinc-800 flex flex-col text-white">
      {/* Main Navbar */}
      <nav className="w-full h-10 md:h-14 md:text-xl text-lg flex justify-center items-center text-yellow-500 font-bold bg-zinc-800 shadow-xl">
        {/* Title */}
        <h1>پیامرسان</h1>
      </nav>
      <div className="h-full grid grid-rows-1 grid-cols-1 md:grid-cols-4">
        {/* Sidebar */}
        <section className="md:col-span-1 col-span-4 h-full overflow-y-auto md:border-l md:border-zinc-700">
          {/* Conversations header */}
          <header className="w-full h-5 hidden md:flex justify-center items-center my-2 text-yellow-500 text-sm font-bold">
            <h2>مکالمه ها</h2>
          </header>
          {/* Conversation Cards list */}
          <ul className="list-none flex flex-col cursor-pointer">
            {/* card */}
            <li className="flex justify-start items-center border-b-2 border-zinc-700 h-24 p-4">
              <Image
                src={Avatar.src}
                alt="Avatar"
                priority
        ...
        {/* Chat Section */}
        <section className="hidden md:block md:col-span-3 h-full">
          {children}
        </section>
      </div>
    </main>
